...
import com.itisacat.com.common.core.config.CcExceptionConfig;
    ...
    public static SysException exportException(Exception ex){
        ...
        if(
                ex instanceof NumberFormatException
                        ...
                        || ex instanceof HJErrorRequestException
                ){
...
//
        if(!(ex instanceof SysException || ex instanceof AppException)) {
            if(flag) {
                log.error("", ex);
                //方法不支持和丢失body暂时不报警
                if((!(ex instanceof HttpRequestMethodNotSupportedException)) && (!(ex instanceof HttpMediaTypeNotSupportedException)) && (!(ex instanceof HttpMessageNotReadableException))){
                }
            }
        }
